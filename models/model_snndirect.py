...
class mid_vgg_direct_residual(nn.Module):
    def __init__(self, max_t, n_class=10):
        super(mid_vgg_direct_residual, self).__init__()
        self.scale = max_t
        ...
        self.fc_1 = t_Linear(4 * 4 * 32*ratio, n_class, bias=False, max_spike_time=self.max_z*4)
    ...
    def forward(self, inp, less_than_first = False):
        ...
        # plt.xlim(0,4.0)
        # plt.ylim(0,200)
        # plt.hist(t.flatten().data.cpu().numpy(), bins=150)
        # plt.savefig('figs/lastlayer.png')
        # exit()
...
class mid_shufflenet_direct(nn.Module):
    def __init__(self, max_t, t_init=0.5, n_class= 10, img_res = 32):
        super(mid_shufflenet_direct, self).__init__()
        self.scale = max_t
        self.max_z = (np.exp(max_t))
        ratio=1
        ...
        self.fc_1 = t_Linear(img_res//8 * img_res//8 * (32+64)*ratio, n_class, bias=False, max_spike_time=self.max_z)
    ...
    def forward(self, inp, less_than_first=False):
        ...
        z3 = self.conv3(z2)
        w3 = self.conv3.kernel.w_sum_cost()
        pre_min = -nn.AvgPool2d(3,stride=2,padding=0)(-nn.ConstantPad2d(1, 1e5)(x1))# * torch.exp(torch.clamp(self.t_shift1, min =0))
...
class mid_vgg_direct(nn.Module):
    def __init__(self, max_t):
        super(mid_vgg_direct, self).__init__()
        self.scale = max_t
        self.max_z = (np.exp(max_t))
        ratio = 2
        self.conv1 = nn.Conv2d(1, 16*ratio, 5, padding=2, bias=False)
        ...
        self.fc_1 = t_Linear(4 * 4 * 32*ratio, 10, bias=False, max_spike_time=self.max_z)
        # self.fc_out = t_Linear(64,10,bias=False)
    ...
    def forward(self, inp, less_than_first = False):
        ...
        # print ((z2==self.max_z).sum()/torch.numel(z2) )
        ...
        z4 = self.conv4(z3)
        w4 = self.conv4.kernel.w_sum_cost()
        # print ((z4==self.max_z).sum()/torch.numel(z4) )

        # draw_hist(z4, 'conv4')

        z5 = self.conv5(z4)
        w5 = self.conv5.kernel.w_sum_cost()
        # print ((z5==self.max_z).sum()/torch.numel(z5) )
